# SurakṣāMārga.ai — Implementation Document

## 1. Project Overview

**SurakṣāMārga.ai** is a 5G-powered women safety navigation system that recommends the safest walking routes by analyzing 157,160+ real crime records from Bangalore. Unlike Google Maps which optimizes for speed, SurakṣāMārga.ai optimizes for safety — scoring every route segment using a multi-factor AI risk engine, adjusting for time-of-day, and leveraging 5G ultra-low latency for real-time safety monitoring.

**One-line pitch:** *"Google Maps tells you the fastest route. We tell you the safest."*

### 1.1 Advanced Safety Mock Layer (Hackathon Extension)

The system now includes an **advanced mock safety layer** for judge-facing demos:

- Identity verification (Gov ID + face confidence simulation)
- Offline/no-network alert queue with retry/flush on reconnect
- Enhanced SOS escalation (contacts + authority ping + live stream mock)
- Emergency stop intelligence (hospital/police/pharmacy map layer)
- Ride app route sharing simulation (Ola/Uber/Rapido deep-link mocks)
- Bluetooth proximity threat mock using RSSI distance estimation
- Behavioral threat assessment combining proximity + route + time + crime context
- Safety loop closure via explicit "Reached Safely?" completion API
- 4G vs 5G safety comparison with priority SOS channel narrative

Primary API surface for this layer is exposed under `/api/v1/*`.
...
- Functions: `getSafetyRoute()`, `getCrimeZones()`, `sendSOS()`, `resolveSOS()`, `setNetworkMode()`, `getRealtimeRisk()`, `getCrowdDensity()`
...
User types "Koramangala" → "MG Road", taps "Find Safe Routes"
    │
    ▼
[Frontend] useRoutes.fetchRoutes() → api.getSafetyRoute()
    │
...
```bash
docker run -d \
  --name suraksha-marga-db \
  -p 5432:5432 \
  -e POSTGRES_USER=postgres \
  -e POSTGRES_PASSWORD=postgres \
  -e POSTGRES_DB=surakshamarga \
...
|----------|---------|-------------|
| `DATABASE_URL` | `postgresql://postgres:postgres@localhost:5432/surakshamarga` | PostgreSQL connection string |
