...
import IdentityOnboarding from './src/screens/IdentityOnboarding';
...
function AppContent() {
  ...
  if (screen === 'Onboarding') {
    return (
      <IdentityOnboarding
        onVerified={(payload) => {
          setScreen('Map');
          setScreenParams({ userProfile: payload });
        }}
      />
    );
  }
  ...
  return (
    <Box sx={{ minHeight: '100vh', position: 'relative', width: '100%' }}>
      ...
      <HeroSection />
      <BrandMarquee />
      <MapScreen navigation={navigation} route={route} themeMode="dark" />
      <Footer />
    </Box>
  );
}
